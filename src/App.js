...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setFilteredCryptoData(response.data.Data.slice(5, 15));
      } catch (error) {
        console.log(error);
      }
    };
  ...
  const calculatePriceInSelectedCurrency = (coin) => {
    const price = coin.RAW && coin.RAW[selectedCurrency] && coin.RAW[selectedCurrency].PRICE;
    if (price !== undefined) {
  ...
  const calculateMarketCapInSelectedCurrency = (coin) => {
    const marketCap = coin.RAW && coin.RAW[selectedCurrency] && coin.RAW[selectedCurrency].MKTCAP;
  ...
  const calculateDirectVolInSelectedCurrency = (coin) => {
    const volume24hTo = coin.RAW && coin.RAW[selectedCurrency] && coin.RAW[selectedCurrency].VOLUME24HOURTO;
  ...
  const calculateTotalVolInSelectedCurrency = (coin) => {
    const totalVolume24hTo = coin.RAW && coin.RAW[selectedCurrency] && coin.RAW[selectedCurrency].TOTALVOLUME24HTO;
            ...
            <tbody>
              {filteredCryptoData.map((coin) => (
                <tr key={coin.CoinInfo.Id}>
                  ...
                  <td className="full-volume">
                    {getCurrencySymbol(selectedCurrency).TOTALTOPTIERVOLUME24HTO}
